...
def current_calculation(matrix):
    ...
    combinations(range(player_count), 2)
    return p

def run_test_rounds_n_correlation(matrix_size, rounds=3):
	player_strengths = np.random.rand(matrix_size)
...
def run_test_rounds_many_correlation(matrix_size, n_iter):
	correlations = []
	for _ in tqdm(range(n_iter)):
		correlation = run_test_rounds_n_correlation(matrix_size, 3)
...
if __name__ == '__main__':
    run_test_rounds_many_correlation(12, 1000)
